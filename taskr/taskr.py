...
def usage(commands):
    print 'usage: taskr <command> [<args>]'
    print ''
...
def __get_commands():
    ...
    for filepath in os.listdir(dirname):
        match = re.match(r'taskr_(.*)\.py', filepath)

        if not match:
            continue

        name = match.group(1)
        module_name = 'taskr_%s' % name
        module = getattr(__import__('taskr.%s' % module_name), module_name)
